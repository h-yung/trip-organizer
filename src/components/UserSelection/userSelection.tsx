	...
	const assignUser = useCallback(
		(event: any) => {
			...
			const activeUser = userOptions.find(
				(opt: User) => opt._id === event.currentTarget.id
			);
			...
			if (activeUser) {
				...
				activeUser.avatar = avatarDictionary.find(
					({ ref }: AvatarItem) => ref === activeUser?.avatarRef
				)?.svg;
				...
				<div className="user-selection-ctr">
					{userOptions.map(
						({ _id, displayName, avatarRef }: User) => {
							const svg = avatarDictionary.find(
								({ ref }: AvatarItem) => ref === avatarRef
							)?.svg;
							return (
								<Link to="/trip" key={_id}>
									<div
